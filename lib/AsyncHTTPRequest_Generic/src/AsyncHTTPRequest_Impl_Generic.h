/****************************************************************************************************************************
  AsyncHTTPRequest_Impl_Generic.h - Dead simple AsyncHTTPRequest for ESP8266,
 ESP32 and currently STM32 with built-in LAN8742A Ethernet

  For ESP8266, ESP32 and STM32 with built-in LAN8742A Ethernet (Nucleo-144,
 DISCOVERY, etc)

  AsyncHTTPRequest_STM32 is a library for the ESP8266, ESP32 and currently STM32
 run built-in Ethernet WebServer

  Based on and modified from asyncHTTPrequest Library
 ...
 along with this program. If not, see <https://www.gnu.org/licenses/>.
  ...
  ------- -----------  ---------- -----------
  1.0.0    K Hoang     14/09/2020 Initial coding to add support to STM32 using
 built-in Ethernet (Nucleo-144, DISCOVERY, etc).
  ...
  1.7.0    K Hoang     10/02/2022 Add support to new ESP32-S3. Add LittleFS
 support to ESP32-C3. Use core LittleFS 1.7.1    K Hoang     25/02/2022 Add
 example AsyncHTTPRequest_ESP_Multi to demo connection to multiple addresses
  1.8.0    K Hoang     13/04/2022 Add support to ESP8266 using W5x00 with
 lwip_W5100 or lwip_W5500 library 1.8.1    K Hoang     13/04/2022 Add support to
 ESP8266 using ENC28J60 with lwip_enc28j60 library 1.8.2    K Hoang 10/08/2022
 Fix library.properties to remove unavailable items from depends 1.9.0    K
 Hoang     30/08/2022 Fix bug. Improve debug messages. Optimize code 1.9.1    K
 Hoang     09/09/2022 Fix ESP32 chipID for example
 `AsyncHTTPRequest_ESP_WiFiManager` 1.9.2    K Hoang     18/10/2022 Not try to
 reconnect to the same host:port after connected 1.10.0   K Hoang     20/10/2022
 Fix bug. Clean up 1.10.1   K Hoang     21/10/2022 Fix bug of wrong reqStates
  1.10.2   K Hoang     09/11/2022 Default to reconnect to the same host:port
 after connected for new HTTP sites 1.11.0   K Hoang     28/11/2022 Add support
 to ESP32 boards using LwIP ENC28J60 Ethernet 1.12.0   K Hoang     30/11/2022
 Add support to ESP32 boards using LwIP W5500 Ethernet. Fix bug 1.13.0   K Hoang
 01/02/2023 Add support to ESP32 using LwIP W6100 Ethernet. Fix _parseURL() bug
 *****************************************************************************************************************************/
...
xbuf::xbuf(const uint16_t segSize)
    : _head(nullptr), _tail(nullptr), _used(0), _free(0), _offset(0) {
...
size_t xbuf::write(const uint8_t* buf, const size_t len) {
  ...
  while (supply) {
    ...
    memcpy(_tail->data + ((_offset + _used) % _segSize), buf + (len - supply),
           demand);
    _free -= demand;
    _used += demand;
    supply -= demand;
...
size_t xbuf::available() { return _used; }
  ...
  while (searchPos <= searchEnd) {
    ...
    if (compLen <= (_segSize - segPos)) {
      ...
      if (memcmp(target, seg->data + segPos, compLen) == 0) {
        ...
        if (memcmp(target + targetLen - compLen, seg->next->data, compLen) ==
            0) {
          return searchPos - _offset;
        }
      }
...
void xbuf::addSeg() {
  if (_tail) {
    ...
    if (_tail == NULL) AHTTP_LOGERROR(F("xbuf::addSeg: error new 2"));
  }
...
bool AsyncHTTPRequest::debug() { return (_debug); }
...
bool AsyncHTTPRequest::open(const char* method, const char* URL) {
  ...
  if (_client && _client->connected()) {
    if ((strcmp(_URL->host, _connectedHost) == 0) &&
        (_URL->port == _connectedPort)) {
  ...
  if (_client && _client->connected() &&
      (strcmp(_URL->host, _connectedHost) != 0 ||
       _URL->port != _connectedPort)) {
...
void AsyncHTTPRequest::setTimeout(int seconds) { _timeout = seconds; }
...
reqStates AsyncHTTPRequest::readyState() { return _readyState; }

////////////////////////////////////////

int AsyncHTTPRequest::responseHTTPcode() { return _HTTPcode; }
...
/*______________________________________________________________________________________________________________
               ...
               PPPP    RRRR     OOO    TTTTT   EEEEE    CCC    TTTTT   EEEEE
  DDDD P   P   R   R   O   O     T     E       C   C     T     E       D   D
               PPPP    RRRR    O   O     T     EEE     C         T     EEE     D
  D P       R  R    O   O     T     E       C   C     T     E       D   D P R R
  OOO      T     EEEEE    CCC      T     EEEEE   DDDD
...
bool AsyncHTTPRequest::_parseURL(const String& url) {
  ...
  if (_URL) {
    ...
    if (!(_URL->scheme)) return false;
  } else
    return false;

  strcpy(_URL->scheme, ASYNC_HTTP_PREFIX);

  if (url.substring(0, strlen(ASYNC_HTTP_PREFIX))
          .equalsIgnoreCase(ASYNC_HTTP_PREFIX)) {
    hostBeg += strlen(ASYNC_HTTP_PREFIX);
  } else if (url.substring(0, strlen(ASYNC_HTTPS_PREFIX))
                 .equalsIgnoreCase(ASYNC_HTTPS_PREFIX)) {
    return false;
  }
  ...
  if (_URL->host == nullptr) return false;
...
bool AsyncHTTPRequest::_connect() {
  ...
  if (_connectedHost == nullptr) return false;

  strcpy(_connectedHost, _URL->host);
  _connectedPort = _URL->port;

  _client->onConnect(
      [](void* obj, AsyncClient* client) {
  ...
  _client->onError(
      [](void* obj, AsyncClient* client, uint32_t error) {
  ...
  if (!_client->connected()) {
    ...
    if (!_client->connect(_URL->host, _URL->port)) {
      AHTTP_LOGERROR3(F("client.connect failed:"), _URL->host, F(","),
                      _URL->port);
...
size_t AsyncHTTPRequest::_send() {
  ...
  if (!_client->connected()) {
    // KH fix bug
    // https://github.com/khoih-prog/AsyncHTTPRequest_Generic/issues/38
    _timeout = DEFAULT_RX_TIMEOUT;
...
void AsyncHTTPRequest::_processChunks() {
  while (_chunks->available()) {
    size_t _chunkRemaining =
        _contentLength - _contentRead - _response->available();
...
/*______________________________________________________________________________________________________________
  ...
  EEEEE   V   V   EEEEE   N   N   TTTTT         H   H    AAA    N   N   DDDD L
  EEEEE   RRRR     SSS E       V   V   E       NN  N     T           H   H   A
  A   NN  N   D   D   L       E       R   R   S EEE     V   V   EEE     N N N T
  HHHHH   AAAAA   N N N   D   D   L       EEE     RRRR     SSS E        V V    E
  N  NN     T           H   H   A   A   N  NN   D   D   L       E       R  R S
  EEEEE     V     EEEEE   N   N     T           H   H   A   A   N   N   DDDD
  LLLLL   EEEEE   R   R    SSS
  _______________________________________________________________________________________________________________*/
...
void AsyncHTTPRequest::_onConnect(AsyncClient* client) {
  ...
  _client->onAck(
      [](void* obj, AsyncClient* client, size_t len, uint32_t time) {
        (void)client;
        (void)len;
        (void)time;
  ...
  _client->onData(
      [](void* obj, AsyncClient* client, void* data, size_t len) {
        (void)client;
...
void AsyncHTTPRequest::_onDisconnect(AsyncClient* client) {
  ...
  if (_readyState < readyStateOpened) {
    _HTTPcode = HTTPCODE_NOT_CONNECTED;
             ...
             (_readyState < readyStateHdrsRecvd ||
              (_contentRead + _response->available()) < _contentLength)) {
...
void AsyncHTTPRequest::setReqHeader(const char* name,
...
void AsyncHTTPRequest::setReqHeader(const __FlashStringHelper* name,
                                    const char* value) {
  if (_readyState <= readyStateOpened && _headers) {
...
AsyncHTTPRequest::header* AsyncHTTPRequest::_addHeader(const char* name,
                                                       const char* value) {
  MUTEX_LOCK(nullptr)
