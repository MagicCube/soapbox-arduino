/****************************************************************************************************************************
  AsyncHTTPRequest_Generic.hpp - Dead simple AsyncHTTPRequest for ESP8266, ESP32
 and currently STM32 with built-in LAN8742A Ethernet
...
#define MUTEX_LOCK_NR                                                 \
  if (xSemaphoreTakeRecursive(threadLock, portMAX_DELAY) != pdTRUE) { \
    return;                                                           \
  }
...
#define _AHTTP_unlock
       ...
       defined(STM32H7) || defined(STM32G0) || defined(STM32G4) || \
...
class xbuf : public Print {
 public:
  ...
  /* In addition to the above functions, the following inherited functions from
  the Print class are available.

  size_t printf(const char * format, ...)  __attribute__ ((format (printf, 2,
  3))); size_t printf_P(PGM_P format, ...) __attribute__((format(printf, 2,
  3))); size_t print(const __FlashStringHelper *); size_t print(const String &);
...
class AsyncHTTPRequest {
  ...
  typedef std::function<void(void*, AsyncHTTPRequest*, int readyState)>
      readyStateChangeCB;
  typedef std::function<void(void*, AsyncHTTPRequest*, size_t available)>
      onDataCB;
 ...
 public:
  ...
  void onReadyStateChange(
      readyStateChangeCB,
      void* arg = 0);    // Optional event handler for ready state change or you
                         // can simply poll readyState()
  void setTimeout(int);  // overide default timeout (seconds)

  void setReqHeader(const char* name,
                    const char* value);  // add a request header
  void setReqHeader(const char* name,
                    int32_t value);  // overload to use integer value
  ...
  void setReqHeader(const __FlashStringHelper* name,
  ...
  bool send(const uint8_t* buffer,
            size_t len);              // Send the request (POST) (binary data?)
  ...
  size_t
  responseLength();        // indicated response length or sum of chunks to date
  ...
  size_t responseRead(uint8_t* buffer,
                      size_t len);  // Read response into buffer
  uint32_t elapsedTime();  // Elapsed time of in progress transaction or last
                           // completed (ms)
 ...
 private:
  ...
  const char* _HTTPmethodStringwithSpace[HTTPmethodMAX] = {
  ...
  readyStateChangeCB
      _readyStateChangeCB;       // optional callback for readyState change
  ...
  // request and response String buffers and header list (same queue for request
  // and response).

  xbuf* _request;    // Tx data buffer
  xbuf* _response;   // Rx data buffer for headers
  xbuf* _chunks;     // First stage for chunked response
  header* _headers;  // request or (readyState > readyStateHdrsRcvd) response
                     // headers
